...
# Функция готова
def update_data_item(request_body, data_item_id, headers=None):
    if headers is None:
        headers = HEADERS
...
# Функция готова
def get_data_item(data_collection_id, data_item_id, headers=None):
    if headers is None:
        headers = HEADERS
...
# Функция готова, в теле запроса нужно передать список референтных полей
...
# DEFAULT_RESEARCH = [research['data']['clinicalStudiesName'] for research in query_data_items(
# COLLECTION_RESEARCHES_BODY)['dataItems']]
...
# DEFAULT_PHASES = [phase['data']['researchPhasesName'] for phase in query_data_items(COLLECTION_PHASES_BODY)[
# 'dataItems']]
...
# DEFAULT_METHODS = [method['data']['contactPreferencesName'] for method in query_data_items(
# COLLECTION_METHODS_BODY)['dataItems']]
